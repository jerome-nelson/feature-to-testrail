function parser (data, args) {
  const {debug} = args;
  const {devlog} = require ('./logger') ('[Parser]::', debug);
  ...
  const getDescription = input => {
    ...
    if (!title) {
      devlog ('getDescription - ERROR: Result not defined');
      return void 0;
    }
  ...
  function formatData (input) {
    ...
    for (let i = 1; i <= sections.length; i += 2) {
      ...
      const firstTag = sections[i].split (regLib.tags)[1];
      const isSmoke = (firstTag && firstTag.toLowerCase().trim() === '@smoke');
      output.push ({
        Steps,
        Expected,
        Title,
